...
export const requiredIf = (condition: (allValues: any) => boolean): FieldValidator<any> => (value, allValues) => {
...
export const minLength = <T = any>(min: number): FieldValidator<T> => value =>
  _.size(String(value)) < min ? error('MinLength', { min }) : undefined

export const maxLength = <T = any>(max: number): FieldValidator<T> => value =>
  _.size(String(value)) > max ? error('MaxLength', { max }) : undefined

export const length = <T = any>({ min, max }: { min?: number; max?: number }): FieldValidator<T> => (
  value,
  values,
  meta
) => {
...
export const regex = <T = any>(regex: RegExp, message: string): FieldValidator<T> => (value, values, meta) => {
...
export const regexOrEmpty = <T = any>(regex: RegExp, message: string): FieldValidator<T> => value => {
